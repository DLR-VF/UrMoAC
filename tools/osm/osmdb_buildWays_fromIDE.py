...
def commit_roads(conn, cursor, schema, dbprefix, area, mode):
...
def createTables(db, conn, cursor, dschema, dprefix, area, mode, append, dropprevious):
    if db.table_exists(dschema, dprefix+"_"+area+"_"+mode+"_network"):
        if dropprevious:
            cursor.execute("""DROP TABLE %s.%s_%s_%s_network;""" % (dschema, dprefix, area, mode))
            conn.commit()
        elif not append:
    ...
    if not append:
        cursor.execute("""CREATE TABLE %s.%s_%s_%s_network (
            ...
            name text,
            slope real    
        );""" % (dschema, dprefix, area, mode))
        cursor.execute("""SELECT AddGeometryColumn('%s', '%s_%s_%s_network', 'geom', 4326, 'LINESTRING', 2);""" % (dschema, dprefix, area, mode))
        conn.commit()


def createStationTable(db, conn, cursor, dschema, dprefix, area, mode, append, dropprevious):
    if db.table_exists(dschema, dprefix + "_" + area + "_" + mode + "_stations"):
        if dropprevious:
            cursor.execute("""DROP TABLE %s.%s_%s_%s_stations;""" % (dschema, dprefix, area, mode))
            conn.commit()
        elif not append:
            print("osmdb_buildWays: error: destination table already exist", file=sys.stderr)
            return 1
    if not append:
        ...
        cursor.execute(
            """SELECT AddGeometryColumn('%s', '%s_%s_%s_stations', 'geom', 4326, 'POINT', 2);""" % (dschema,
                                                                                                    ...
                                                                                                    mode))
        conn.commit()
    if db.table_exists(dschema, dprefix + "_" + area + "_" + mode + "_stops"):
        cursor.execute("""DROP TABLE %s.%s_%s_%s_stops;""" % (dschema, dprefix, area, mode))
        conn.commit()
    ...
    cursor.execute(
        """SELECT AddGeometryColumn('%s', '%s_%s_%s_stops', 'geom', 4326, 'POINT', 2);""" % (dschema, dprefix, area,
                                                                                             mode))
    conn.commit()

def fillStopsTable(db, conn, cursor, dschema, dprefix, area, mode):
    rest = db.getStops(area, mode, dschema, dprefix)
    print(type(rest))
    print('Stops queried.')
    stationlist = []
    stoplist = []
    ...
    for st in rest:
        ...
        cursor.execute(
            "INSERT INTO %s.%s_%s_%s_stops(id, k, v, geom) VALUES(%s, '%s', '%s', ST_GeomFromEWKB(decode('%s', 'hex')));" % (
                dschema, dprefix, area, mode, st[0], st[1], st[2], st[3]))
    conn.commit()
    return [stationlist, stoplist, haltlist]
    print('Stops temporarily stored in database.')

def aggregateStops(db, conn, cursor, dschema, dprefix, area, mode, stationstophaltlist):
    seen = []
    stationcounter = 0
    ...
    for ssh in [stationlist, stoplist, haltlist]:
        for elem in ssh:
            if elem not in seen:
                ...
                #    seen.append(elem)
                mainstop = db.fetchCloseStops(elem, area, mode, dschema, dprefix)
                ...
                for i in mainstop:
                    ...
                    if i[2] != 'station':
                        seen.append(i[0])
                cursor.execute(
                    "INSERT INTO %s.%s_%s_%s_stations(id, stationsname, category, sub_stops, geom) VALUES(%s, '%s', '%s', ARRAY%s, ST_GeomFromEWKB(decode('%s', 'hex')));" % (
                        dschema, dprefix, area, mode, mainstop[0][0], stationname[0][0], mainstop[0][2],
                        stationsub_stops, mainstop[0][3]))
                stationcounter += 1
                conn.commit()
    cursor.execute("DROP TABLE %s.%s_%s_%s_stops;" % (dschema, dprefix, area, mode))
    conn.commit()
...
# --- main
def build_ways(src_db, dest_db, dropprevious, append, unconsumed_file, errorneous_file, verbose, area, mode):
    """Main method"""
    ...
    createTables(db, conn, cursor, dschema, dprefix, area, mode, append, dropprevious)
    ...
    IDs = db.getWayKV_withMatchingKey(upperType, area)
    highways = get_highways(mode)
    # get nodes usage
    ...
    for i,h in enumerate(IDs):
        ...
        if len(storedRoads)>10000:
            commit_roads(db._conn, db._cursor, dschema, dprefix, area, mode)
            print('Committed 10k ways.')
    commit_roads(db._conn, db._cursor, dschema, dprefix, area, mode)
    cursor.execute("UPDATE %s.%s_%s_%s_network SET length=ST_Length(geom::geography);" % (dschema, dprefix, area, mode))
    conn.commit()
    ...
    if mode == 'railway' or 'local_pt':
        ...
        createStationTable(db, conn, cursor, dschema, dprefix, area, mode, append, dropprevious)
        #Die temporäre StopTable wird befüllt mit den Stationen, stops und halten in der area und dem modus. diese abfrage
        #dauert einen moment, spart aber beim vergleichen und eliminieren von stops sehr viel zeit
        stationstophaltlist = fillStopsTable(db, conn, cursor, dschema, dprefix, area, mode)
        ...
        stationcount = aggregateStops(db, conn, cursor, dschema, dprefix, area, mode, stationstophaltlist)

        print(str(stationcount)+' stations were added.')
...
build_ways("vf-athene.intra.dlr.de,project_indikatoren_fuss,osm.germany20250516,bras_ml,2kbV296nCa", "vf-athene.intra.dlr.de,project_indikatoren_fuss,osmextract.test,bras_ml,2kbV296nCa",
           True, False, None, None, False, "hamburg", "railway")
