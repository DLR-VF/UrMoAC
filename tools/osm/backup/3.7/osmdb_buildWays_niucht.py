            ...
            if elem not in seen:
                ...
                for i in mainstop:
                    ...
                    cursor.execute("""
                                SELECT v FROM
                                osm.germany20250516_rtag
                                where id = '%s' and k = 'ref';""" %(i[0]))
                    linehalt = cursor.fetchall()
                    if linehalt:
                        if linehalt[0][0] not in linehalts:
                            linehalts.append(linehalt[0][0])

                cursor.execute(
                    "INSERT INTO %s.%s_%s_%s_stations(id, stationsname, stationtype, category, sub_stops, linelist, geom) VALUES(%s, '%s', '%s', '%s', ARRAY%s, ARRAY%s::text[],  ST_GeomFromEWKB(decode('%s', 'hex')));" % (
                        dschema, dprefix, region, mode, mainstop[0][0], stationname[0][0], stationtype, mainstop[0][2],
                        stationsub_stops, linehalts, mainstop[0][3]))
                stationcounter += 1
                conn.commit()
