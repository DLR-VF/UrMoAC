...
def stationLineAdder(db, conn, cursor, dschema, dprefix, region, mode, stationID):
    ...
    linelist=[]
    cur.execute(
        """SELECT sub_stops from osmextract.test_berlin_railway_stations where id ='%s';""" % stationID)
    ...
    for ssh in [stationlist, stoplist, haltlist]:
        for elem in ssh:
            if elem not in seen:
                ...
                cursor.execute(
                    "INSERT INTO %s.%s_%s_%s_stations(id, stationsname, stationtype, category, sub_stops, geom) VALUES(%s, '%s', '%s', '%s', ARRAY%s,  ST_GeomFromEWKB(decode('%s', 'hex')));" % (
                        dschema, dprefix, region, mode, mainstop[0][0], stationname[0][0], stationtype, mainstop[0][2],
                        stationsub_stops,  mainstop[0][3]))
                stationcounter += 1
                conn.commit()
                linelist=stationLineAdder(db, conn, cursor, dschema, dprefix, region, mode, elem)
                print(linelist)
                cursor.execute(
                    "UPDATE %s.%s_%s_%s_stations SET linelist = ARRAY%s::text[] WHERE id ='%s'"
                     % (
                        dschema, dprefix, region, mode, linelist, elem))
                conn.commit()
    cursor.execute("DROP TABLE %s.%s_%s_%s_stops;" % (dschema, dprefix, region, mode))
    conn.commit()
...
#def establishConnection(x_db):


def build_pois(src_db, dest_db, dropprevious, append, unconsumed_file, errorneous_file, verbose, region, mode):
    ...
    print('Determining key value pairs.')
    kvpairs = get_kvpairs(mode)
    kvpairlist=list(kvpairs.keys())
    for transformindex in range(len(kvpairlist)):
        kvpairlist[transformindex] = kvpairlist[transformindex].split("_")
    for posindex in range(len(kvpairlist)):
        print('Working on list: '+kvpairlist[posindex][0]+":"+kvpairlist[posindex][1])
        IDs=db.getNodeKV_withMatchingKeyValue( kvpairlist[posindex][0], kvpairlist[posindex][1])
        for element in range(len(IDs)):
            id = IDs[element][0]
            k=IDs[element][1]
            v=IDs[element][2]
            geom=db.get_node(id, region)
            if not geom:
                continue
            else:
                geom=geom[0][1]
                name = db.getNodeValue_withKeyAndID(id, 'name')
                if name:
                    name = name[0][0]
                    if '\'' in name:
                        name = 'Bitte pruefen'
                cursor.execute(
                    "INSERT INTO %s.%s_%s_%s_network(id, k, v, name, geom) VALUES(%s, '%s', '%s', '%s', ST_GeomFromEWKB(decode('%s', 'hex')));" % (
                        dschema, dprefix, region, mode, id, k, v, name, geom))
   ...
   #nodes = db.getNodeKV_withMatchingKeyValue(key, value)
...
def build_ways(src_db, dest_db, dropprevious, append, unconsumed_file, errorneous_file, verbose, region, mode):
    ...
    if mode in ['railway', 'local_pt']:
        ...
        dt=t3-t2
        print(str(stationcount)+' stations were added in an additional ' + str(dt) )
...
def main(arguments=None):
    ...
    if args.mode is None:
        args.mode = 'railway'
    elif args.mode not in ['highway', 'railway', 'local_pt', 'healthcare']:
        errors.append(
            "Mode must be \'railway\',\'local_pt\', \'highway\' or \'healthcare\'")
    # - output db
