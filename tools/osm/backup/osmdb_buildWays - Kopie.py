...
def commit_roads(conn, cursor, schema, dbprefix):
    ...
    for sr in storedRoads:
        ...
        if (sr["modes"]&PASSENGER)!=0:
            ...
            mode_mit = 'f'
        cursor.execute("INSERT INTO %s.%s_network3(oid, nodefrom, nodeto, numlanes, length, vmax, street_type, capacity, mode_walk, mode_bike, mode_pt, mode_mit, modes, nodes, sidewalk, cycleway, surface, lit, name, geom) VALUES('%s', %s, %s, %s, %s, %s, '%s', -1, '%s', '%s', '%s', '%s', %s, '{%s}', '%s', '%s', '%s', '%s', '%s', ST_GeomFromText('LINESTRING(%s)', 4326))" % (schema, dbprefix, sr["id"], sr["fromNode"], sr["toNode"], sr["lanes"], -1, sr["vmax"], sr["osm_type"], mode_ped, mode_bic, mode_pt, mode_mit, sr["modes"], sr["nodes"], sr["sidewalk"], sr["cycleway"], sr["surface"], sr["lit"], sr["name"], sr["shape"]))
    conn.commit()
    del storedRoads[:]
...
def build_ways(src_db, dest_db, dropprevious, append, unconsumed_file, errorneous_file, verbose):
    ...
    # (re) create tables
    if ddb.table_exists(dschema, dprefix+"_network3"):
        if dropprevious:
            cursor.execute("""DROP TABLE %s.%s_network3;""" % (dschema, dprefix))
            conn.commit()
        elif not append:
            print ("osmdb_buildWays: error: destination table already exist", file=sys.stderr)
            return 1
    if not append:
        cursor.execute("""CREATE TABLE %s.%s_network3 (
        ...
        cursor.execute("""SELECT AddGeometryColumn('%s', '%s_network3', 'geom', 4326, 'LINESTRING', 2);""" % (dschema, dprefix))
        conn.commit()
    ...
    for upperType in ["highway", "railway"]:
        ...
        for i,h in enumerate(IDs):
            ...
            if len(storedRoads)>10000:
                commit_roads(ddb._conn, ddb._cursor, dschema, dprefix)
    commit_roads(ddb._conn, ddb._cursor, dschema, dprefix)
    cursor.execute("UPDATE %s.%s_network3 SET length=ST_Length(geom::geography);" % (dschema, dprefix))
    conn.commit()
