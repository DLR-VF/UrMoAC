...
class Params:
    ...
    def selector(self, category, area):
        return
...
area = 'bremen'  # derzeit keine auswirkungen, suche schönes osm shapefile mit boundaries (nuts3?)
...
def build_ways(src_db, dest_db, dropprevious, append, unconsumed_file, errorneous_file, verbose, area, mode):
    ...
    # (re) create tables
    if ddb.table_exists(dschema, dprefix+"_"+area+"_"+mode+"_network"):
        if dropprevious:
    ...
    for i,h in enumerate(IDs):
        ...
        if len(storedRoads)>10000:
            commit_roads(ddb._conn, ddb._cursor, dschema, dprefix, area, mode)
    commit_roads(ddb._conn, ddb._cursor, dschema, dprefix, area, mode)
...
def main(arguments=None):
    ...
    parser.add_argument('-e','--area', help='Defines the Area (state or NUTS3) in which the system is build. Defaults to \'germany\' as a whole.')
    parser.add_argument('-m', '--mode', help='Defines the mode of transportation (\'railway\', \'local_rail\' or \'street\').')
    ...
    gegenden = ['baden-wuerttemberg', 'bayern', 'berlin', 'brandenburg', 'bremen', 'hamburg', 'hessen', 'mecklenburg-vorpommern',
                'niedersachsen', 'nordrhein-westfalen', 'rheinland-pfalz', 'saarland', 'sachsen', 'sachsen-anhalt', 'schleswig-holstein', 'thueringen' ]
    if args.area is None:
        args.area = 'germany'
    elif args.area not in gegenden:
        errors.append(
            "Area chosen not available;\n must be \'germany\', empty or in: " + gegenden)

    if args.mode is None:
        args.mode = 'railway'
    elif args.mode not in ['highway', 'railway', 'local_rail']:
        errors.append(
            "Mode must be \'railway\',\'local_rail\' or \'highway\'")
    # - output db
    if args.output is not None:
        ...
        output[2] = output[2] + "_"+area+"_"+args.mode+"_network"
        output = ",".join(output)
    # - report
    if len(errors)!=0:
    ...
    # build
    #return build_ways(args.OSMdatabase, output, args.dropprevious, args.append, args.unconsumed_output, args.errorneous_output, args.verbose, args.area, args.mode)
    build_ways(args.OSMdatabase, output, args.dropprevious, args.append, args.unconsumed_output, args.errorneous_output,
               args.verbose, args.area, args.mode)
