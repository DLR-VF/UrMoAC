...
class OSMDB:
    ...
    def getWayKV_withMatchingKey(self, key, area):
        ...
        if area == 'germany' :
            ...
            INNER JOIN (SELECT * FROM osmextract.area WHERE land = '%s') as zelle
            on ST_Within(waynodes.geom, zelle.geom);""" % (self._schema, self._dbprefix, key, self._schema, self._dbprefix, self._schema, self._dbprefix, area)
            print(query)
            return self._execute_fetch_all(query)
    ...
    def lightrailsort(self, lrID):
        query = """SELECT ST_Within(waynodes.geom, zelle.geom)
                    ...
                    on ST_Within(waynodes.geom, zelle.geom);
                    """ % (self._schema, self._dbprefix, lrID, self._schema, self._dbprefix, )
        if self._execute_fetch_all(query)[0][0] is True:
            return 'light_rail'
        else:
            return 'stadtbahn'
