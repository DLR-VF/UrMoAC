...
def build_ways(src_db, dest_db, dropprevious, append, unconsumed_file, errorneous_file, verbose):
    ...
    # (re) create tables
    if db.table_exists(schema, prefix+"_network3"):
        if dropprevious:
            cursor.execute("""DROP TABLE %s.%s_network3;""" % (schema, prefix))
            conn.commit()
        elif not append:
    ...
    if not append:
        ...
        cursor.execute("""SELECT AddGeometryColumn('%s', '%s_network3', 'geom', 4326, 'LINESTRING', 2);""" % (schema, prefix))
        conn.commit()
    ...
    commit_roads(db._conn, db._cursor, schema, prefix)
    cursor.execute("UPDATE %s.%s_network3 SET length=ST_Length(geom::geography);" % (schema, prefix))
    conn.commit()
