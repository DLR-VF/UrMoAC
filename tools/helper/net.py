...
def plotNet(ax, net, color="#000000", alpha=1, widthmap = {
  ...
  for e in edges:
    ...
    if st not in widthmap:
        continue
    ax.add_artist(e.artist(color=color, lw=widthmap[st], zorder=1000))
    
"""    
    if st not in e2rt:
      e2rt[st] = []
    g = e.geometry().getShape()[0]
    for i,p in enumerate(g):
      if i==0:
        continue
      e2rt[st].append([g[i-1], g[i]])
  for st in e2rt:
    if st not in widthmap:
      continue
    coll = LineCollection(e2rt[st], colors=color, linewidths=widthmap[st], alpha=alpha)
    ax.add_collection(coll)

"""

def plot_limts():
  xlim(343231.1126699702, 373140.7778314906)
  ylim(5632496.039618671, 5660811.669274271)
